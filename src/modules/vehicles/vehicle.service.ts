...
async function assertCustomerBelongsToOrganization(
  ...
  const customer =
    await import(
      "../customers/customer.repository.js"
    ).then(
      ({
        findCustomerById,
      }) =>
...
export async function createVehicle(
  ...
  if (input.customerId) {
    await assertCustomerBelongsToOrganization(
      organizationId,
      input.customerId,
    );
  }
...
export async function listVehicles(
  ...
  return findVehiclesByOrganization(
    organizationId,
    query,
  );
}

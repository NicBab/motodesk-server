...
export function assertMembershipCreateAllowed(
...
export function assertMembershipRemovalAllowed(
  actor: MembershipActorContext,
  target: Membership,
  organizationId: string,
): void {
  ...
  if (target.role === MembershipRole.OWNER) {
    throw new AppError(403, "Owners cannot be removed.", {
      code: "OWNER_PROTECTED",
    });
  }

  if (target.id === actor.membershipId) {
    throw new AppError(403, "You cannot remove your own membership.", {
      code: "SELF_MEMBERSHIP_REMOVAL_FORBIDDEN",
    });
  }

...
describe(
  ...
  () => {
    it(
      "creates, retrieves, updates, lists, and changes status with history",
      async () => {
        const {
          agent,
          organizationId,
        ...
        const customerResponse =
          await agent
            ...
            .send({
              ...
              lastName: "Customer",
              email:
                "repair.customer@motodesk.local",
            });
        ...
        const createResponse =
          await agent
            ...
            .send({
              ...
              vehicleId,
              complaint:
                "Customer reports hard starting when hot.",
              priority:
                "STANDARD",
              shopSuppliesRate:
                6,
        ...
        const roNumber =
          createResponse.body.data.roNumber;

        assert.equal(
          typeof repairOrderId,
          "string",
        );
        ...
        assert.equal(
          createResponse.body.data.statusHistory.length,
          1,
        );

        assert.equal(
          createResponse.body.data.statusHistory[0].status,
          "ESTIMATE",
        );
        ...
        assert.equal(
          getResponse.body.data.vehicleId,
          vehicleId,
        );
        ...
        assert.equal(
          updateResponse.body.data.priority,
          "RUSH",
        );
        ...
        const statusResponse =
          await agent
            ...
            .send({
              ...
              notes:
                "Estimate ready for customer review.",
              automatic:
                false,
            });
        ...
        assert.equal(
          statusResponse.body.data.statusHistory.length,
          2,
        );
        ...
        assert.equal(
          duplicateStatusResponse.status,
          400,
        );
    ...
    it(
      "allocates unique sequential RO numbers within an organization",
      async () => {
        const {
          agent,
          organizationId,
        ...
        assert.equal(
          ...
          firstResponse.body.data.roNumber +
            1,
        );
      },
    );

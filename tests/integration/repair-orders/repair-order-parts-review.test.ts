...
async function createPartsReviewFixture(
  options: {
    qtyOnHand: number;
    suffix: string;
  },
...
describe(
  "Repair Order parts review integration",
  () => {
    it(
      "rejects completion while a blocking part still needs review",
      async () => {
        const fixture =
          await createPartsReviewFixture({
            qtyOnHand:
              0,
        ...
        const completeResponse =
          await agent
            ...
            .send({
              notes:
                "Attempting to complete parts review.",
            });
    ...
    it(
      "moves the repair order to WAITING_ON_PARTS when reviewed parts must be ordered",
      async () => {
        const fixture =
          await createPartsReviewFixture({
            qtyOnHand:
        ...
        const toBeOrderedResponse =
          await agent
            ...
            .send({
              notes:
                "Part is not available in shop inventory.",
            });
        ...
        assert.equal(
          toBeOrderedResponse.body.data.status,
          "TO_BE_ORDERED",
        );
        ...
        const completeResponse =
          await agent
            ...
            .send({
              notes:
                "Parts review complete. Required part must be ordered.",
            });
    ...
    it(
      "moves the repair order to READY_TO_WORK when all blocking parts are available from stock",
      async () => {
        const fixture =
          await createPartsReviewFixture({
            qtyOnHand:
        ...
        const completeResponse =
          await agent
            ...
            .send({
              notes:
                "All required parts allocated from shop inventory.",
            });

...
describe(
  "Purchase Order ordering integration",
  () => {
    it(
      "orders a purchase order and synchronizes on-order inventory and linked repair order part lines",
      async () => {
        const fixture =
        ...
        assert.notEqual(
          orderResponse.body.data.orderedAt,
          null,
        );
        ...
        assert.equal(
          Number(
            partResponse.body.data.qtyOnOrder,
          ),
          2,
        );
        ...
        assert.equal(
          roPartLineResponse.body.data.status,
          "ORDERED",
        );

        assert.equal(
          Number(
            roPartLineResponse.body.data.orderedQty,
          ),
          2,
        );
        ...
        assert.equal(
          Number(
            finalPartResponse.body.data.qtyOnOrder,
          ),
          2,
        );
      },

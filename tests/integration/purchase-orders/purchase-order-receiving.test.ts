...
describe(
  ...
  () => {
    it(
      "partially and fully receives a purchase order and releases the repair order when all blocking parts are received",
      async () => {
        const fixture =
        ...
        assert.equal(
          waitingResponse.status,
          200,
        );

        assert.equal(
          waitingResponse.body.data.status,
          "WAITING_ON_PARTS",
        );
        ...
        assert.equal(
          partialReceiveResponse.status,
          200,
        );

        assert.equal(
          partialReceiveResponse.body.data.status,
          "PARTIALLY_RECEIVED",
        );

        assert.equal(
          Number(
            partialReceiveResponse.body.data.lines[0].receivedQty,
          ),
          1,
        );
        ...
        assert.equal(
          Number(
            partAfterPartialReceipt.body.data.qtyOnHand,
          ),
          1,
        );
        ...
        assert.equal(
          Number(
            partAfterPartialReceipt.body.data.qtyOnOrder,
          ),
          1,
        );
        ...
        const roPartLineAfterPartial =
          await agent.get(
            `/api/v1/organizations/${organizationId}/repair-orders/${repairOrderId}/part-lines/${repairOrderPartLineId}`,
          );

        assert.equal(
          roPartLineAfterPartial.body.data.status,
          "PARTIALLY_RECEIVED",
        );

        assert.equal(
          Number(
            roPartLineAfterPartial.body.data.receivedQty,
          ),
          1,
        );
        ...
        const excessiveReceiveResponse =
          await agent
            .post(
              `/api/v1/organizations/${organizationId}/purchase-orders/${purchaseOrderId}/receive`,
            )
        ...
        assert.equal(
          excessiveReceiveResponse.status,
          400,
        );
        ...
        assert.equal(
          finalReceiveResponse.status,
          200,
        );

        assert.equal(
          finalReceiveResponse.body.data.status,
          "RECEIVED",
        );

        assert.notEqual(
          finalReceiveResponse.body.data.receivedAt,
          null,
        );

        assert.equal(
          Number(
            finalReceiveResponse.body.data.lines[0].receivedQty,
          ),
          2,
        );
        ...
        assert.equal(
          Number(
            partAfterFinalReceipt.body.data.qtyOnHand,
          ),
          2,
        );
        ...
        assert.equal(
          Number(
            partAfterFinalReceipt.body.data.qtyOnOrder,
          ),
          0,
        );
        ...
        const roPartLineAfterFinal =
          await agent.get(
            `/api/v1/organizations/${organizationId}/repair-orders/${repairOrderId}/part-lines/${repairOrderPartLineId}`,
          );

        assert.equal(
          roPartLineAfterFinal.body.data.status,
          "RECEIVED",
        );

        assert.equal(
          Number(
            roPartLineAfterFinal.body.data.receivedQty,
          ),
          2,
        );
